...
/*! \file GoState.cc
	\brief GoState interface implementaiton.
...
GoSerial GoState::get_serial () {
	return (general_decode(encode_lr[ENCODE_SERIAL].L, 
	 encode_lr[ENCODE_SERIAL].R));
}
void GoState::get_kopass ( GoCoordId &ko_position, GoCoordId &pass_count ) {
	GoCoordId kopass = general_decode(encode_lr[ENCODE_SERIAL].L, 
	 encode_lr[ENCODE_SERIAL].R);
...
GoCoordId GoState::get_outdeg () {
	return (general_decode(encode_lr[ENCODE_OUTDEG].L, 
	 encode_lr[ENCODE_OUTDEG].R));	
} 
GoCoordId GoState::get_result () {
	return (general_decode(encode_lr[ENCODE_RESULT].L, 
	 encode_lr[ENCODE_RESULT].R));
}
GoScore GoState::get_board_score () {
	return (general_decode(encode_lr[ENCODE_SCORE].L, 
	 encode_lr[ENCODE_SCORE].R));
}
/******************************** Encode *********************************/
...
void GoState::set_kopass ( GoCoordId ko_position, GoCoordId pass_count ) {
	GoCoordId kopass = (pass_count>0) ? 
	 SMALLBOARDSIZE+pass_count : ko_position;
