...
// hash for empty board
extern GoHash zobrist_empty_board;
...
extern std::vector<GoPosition> cached_neighbor_coord[GoConstant::BORDER_R]
                                                    [GoConstant::BORDER_C];
...
#define FOR_NEIGHBOR(id, nb)                                                   \
    for (GoCoordId *nb = cached_neighbor_id[(id)], i = 0;                      \
...
#define REV_FOR_EACH_COORD(id)                                                 \
    for (GoCoordId id = GoConstant::SMALLBOARDSIZE - 1; id >= 0; id--)
